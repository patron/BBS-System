<main class="app-content">

<div class="app-title">
    <div>
        <h1><i class="fa fa-edit"></i> Get All Project Release History</h1>
        <p>Release History of Projects</p>
    </div>
    <ul class="app-breadcrumb breadcrumb">
        <li class="breadcrumb-item"><i class="fa fa-home fa-lg"></i></li>
        <li class="breadcrumb-item">Manager</li>
        <li class="breadcrumb-item"><a href="#">Get All Project Release History</a></li>
    </ul>
</div>

<div class="col-md-12 col-lg-12">
        <div class="tile">
                    <div class="form-group">
                            <label for="projectKeys">Project Keys</label>
                            <select class="form-control" id="projectKeys">
                            </select>
                    </div>
                    <div class="tile-footer">
                        <button class="btn btn-primary js-click-projecthistory">Get Projects Table</button>
                    </div>
        </div>
    </div>
</div>
</main>